...
plot3_dt <- mutate(elpow_data, convDateTime=as.POSIXct(paste(as.character(Date),as.character(Time)), format="%d/%m/%Y %H:%M:%S") ,
                        ...
                        select(convDateTime,safeSub_metering_1,safeSub_metering_2,safeSub_metering_3)  %>%
...
with( plot3_dt, plot(convDateTime, safeSub_metering_1,  type = "l",ylab = "Energy sub metering", xlab = ""))
with( plot3_dt, lines(convDateTime,safeSub_metering_2, col="red"))
with( plot3_dt, lines(convDateTime,safeSub_metering_3, col="blue"))
